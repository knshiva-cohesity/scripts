...
select CONCAT('The DB size based on segments in GiB: ', (select round((sum(bytes)/1024/1024/1024),2) from dba_segments)) from dual;
select CONCAT('The Total bigfile Datasize in GiB: ', (select round((sum(bytes)/1024/1024/1024),2) from (select sum(bytes) bytes from dba_data_files dbf, dba_tablespaces tbsp where dbf.tablespace_name=tbsp.tablespace_name and tbsp.bigfile='YES'))) from dual;
select CONCAT('The Biggest bigfile in GiB: ', (select round((sum(bytes)/1024/1024/1024),2) from (select max(bytes) bytes from dba_data_files dbf, dba_tablespaces tbsp where dbf.tablespace_name=tbsp.tablespace_name and tbsp.bigfile='YES'))) from dual;
select CONCAT('The allocated DB size based on datafile in GiB: ', (select round((sum(bytes)/1024/1024/1024),2) BYTES from v$datafile)) from dual;
...
select CONCAT('The daily redo size in MiB is: ', (select dailyRedoSize from (select round(avg(redo_size/1024/1024),2) dailyRedoSize from (select trunc(completion_time) rundate, sum(blocks*block_size) redo_size from v$archived_log where first_time > sysdate - 7 group by trunc(completion_time))))) from dual;
select CONCAT('The daily change rate in %: ', (select dailyChangeRate from (select (round((avg(redo_size)/sum(sgmt.bytes)),4)*100) dailyChangeRate from containers(cdb_segments) sgmt, (select trunc(completion_time) rundate, sum(blocks*block_size) redo_size from containers(v$archived_log) where first_time > sysdate - 7 group by trunc(completion_time))))) from dual;
...
select CONCAT('SGA Target size in GiB: ', (SELECT round((value/1024/1024/1024),2) from v$parameter where name='sga_target')) from dual;
select CONCAT('PGA Target size in GiB: ', (SELECT round((value/1024/1024/1024),2) from v$parameter where name='pga_aggregate_target')) from dual;
...
select CONCAT('TDE enabled: ', (select decode(count(*), 0, 'NO', 'YES') from dba_encrypted_columns)) from dual;
select CONCAT('TSE enabled: ', (select decode(count(*), 0, 'NO', 'YES') from dba_tablespaces dba, v$tablespace v where dba.encrypted='YES' and dba.tablespace_name=v.name)) from dual;
...
select CONCAT('Encrypted Datasize in MiB: ', (select round(sum(bytes/1024/1024),2) from (select sum(bytes) bytes from v$datafile dbf, v$tablespace v, dba_tablespaces tbsp where dbf.TS#=v.TS# and v.name=tbsp.tablespace_name and tbsp.encrypted='YES'))) from dual;
select CONCAT('FRA configured: ', (select value from v$parameter where name='db_recovery_file_dest')) from dual;
select CONCAT('FRA size in GiB: ', (select round((value/1024/1024/1024),2) from v$parameter where name='db_recovery_file_dest_size')) from dual;
